...
const PRESET_TASKS = [
  { title: '洗碗',     icon: '🍽️', points: 10 },
  { title: '拖地',     icon: '🧹', points: 15 },
  { title: '整理书桌', icon: '📚', points: 10 },
  { title: '倒垃圾',   icon: '🗑️', points: 5  },
  { title: '语文',     icon: '📖', points: 20 },
  { title: '英语',     icon: '🔤', points: 20 },
  { title: '数学',     icon: '🔢', points: 20 },
  { title: '科学',     icon: '🔬', points: 20 },
];
...
function TaskForm({ members, onClose, onSave, currentMember }) {
  const [form, setForm] = useState({
    title: '', description: '', due_date: '', priority: 'medium',
    assigned_to: '', points_value: 10,
  ...
  const applyPreset = preset => setForm(f => ({
    ...f, title: preset.title, points_value: preset.points,
  }));
    ...
    <div className="p-6 space-y-5">
      ...
      {tasks.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {tasks.map(t => <TaskCard key={t.id} task={t} onRefresh={handleRefresh} />)}
        </div>
      )}
